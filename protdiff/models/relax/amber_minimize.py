...
import logging
import sys
# sys.path.append('home/liuyf/alldata/monomer_joint_PriorDDPM_ESM1b_Dnet_LE_MPNN_LC_trans_newmask_20221123/protdiff/models')
from ..folding_af2.common import protein
from ..folding_af2.common import residue_constants
from ..folding_af2 import folding
...
def _add_restraints(
  ...
  logger.info("Restraining %d / %d particles.",
...
def find_violations(prot_np: protein.Protein):
  ...
  np_violations = {
    'between_residues':  {k: v.detach().cpu().numpy() for k, v in violations['between_residues'].items()},
    'within_residues':  {k: v.detach().cpu().numpy() for k, v in violations['within_residues'].items()},
    'total_per_residue_violations_mask': violations['total_per_residue_violations_mask'].detach().cpu().numpy()
  }
  violation_metrics = {k: v.item() for k, v in violation_metrics.items()}
  return np_violations, violation_metrics
...
def _run_one_iteration(
  ...
  while not minimized and attempts < max_attempts:
    attempts += 1
    try:
      logger.info("Minimizing protein, attempt %d of %d.",
...
def run_pipeline(
  ...
  while violations > 0 and iteration < max_outer_iterations:
    ...
    logger.info("Iteration completed: Einit %.2f Efinal %.2f Time %.2f s "
