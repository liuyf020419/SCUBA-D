...
def assert_equal_nonterminal_atom_types(
  ...
  if (isinstance(atom_mask, np.ndarray) and isinstance(ref_atom_mask, np.ndarray)):
    ...
    if isinstance(ref_atom_mask, np.ndarray):
      ref_atom_mask = torch.from_numpy(ref_atom_mask)
    no_oxt_mask = torch.from_numpy(no_oxt_mask)
    np.testing.assert_almost_equal(ref_atom_mask[no_oxt_mask].detach().cpu().numpy(),
                                  atom_mask[no_oxt_mask].detach().cpu().numpy())
