...
class GVPTransformerModel(nn.Module):
    ...
    def cond_sample(self, coords, cond_seq, gap_len, temperature=1.0, confidence=None):
        """
        Samples sequences based on greedy sampling (no beam search).
        ...
        """
        L = len(coords)
        cond_len = len(cond_seq)
        sequence = cond_seq + 'X'*(L-cond_len)
        # Convert to batch format
        batch_converter = CoordBatchConverter(self.decoder.dictionary)
        batch_coords, confidence, _, tokens, padding_mask = (
            batch_converter([(coords, confidence, sequence)])
        )
        # Start with prepend token
        sampled_tokens = torch.zeros(1, 1+L, dtype=int)
        sampled_tokens[0, 0] = self.decoder.dictionary.get_idx('<cath>')
        ...
        for i in range(1, L+1):
            ...
            if i <= cond_len + gap_len:
                sampled_ = tokens[0, i]
            else:
                logits = logits[0].transpose(0, 1)
                logits /= temperature
                probs = F.softmax(logits, dim=-1)
                sampled_ = torch.multinomial(probs, 1).squeeze(-1)
            sampled_tokens[:, i] = sampled_ #torch.multinomial(probs, 1).squeeze(-1)
        sampled_seq = sampled_tokens[0, 1:]
