...
restype_rigidgroup_base_atom37_idx_ = torch.LongTensor(restype_rigidgroup_base_atom37_idx)
restype_rigidgroup_mask_ = torch.FloatTensor(restype_rigidgroup_mask)

restype_rigidgroup_is_ambiguous_ = torch.zeros([21, 8]).float()
restype_rigidgroup_rots_ = torch.eye(3).float()[None, None].repeat(21, 8, 1, 1) #np.tile(np.eye(3, dtype=np.float32), [21, 8, 1, 1])
...
for resname, _ in residue_constants.residue_atom_renaming_swaps.items():
  ...
  restype_rigidgroup_is_ambiguous_[restype, chi_idx + 4] = 1
  restype_rigidgroup_rots_[restype, chi_idx + 4, 1, 1] = -1
  restype_rigidgroup_rots_[restype, chi_idx + 4, 2, 2] = -1
...
def get_chi_atom_indices(device):
  ...
  return torch.LongTensor(chi_atom_indices).to(device)



def atom14_to_atom37(atom14_data,  # (N, 14, ...)
                     batch
                    ):  # (N, 37, ...)
  ...
  atom37_data = torch.gather(atom14_data, 1, batch['residx_atom37_to_atom14'].unsqueeze(-1).repeat(1, 1, 3))
  if len(atom14_data.shape) == 2:
    atom37_data = atom37_data * batch['atom37_atom_exists']
  elif len(atom14_data.shape) == 3:
    atom37_data = atom37_data * batch['atom37_atom_exists'][:, :,
                                               None].type_as(atom37_data)
  return atom37_data
...
def atom37_to_atom14(
  ...
  #                                    batch_dims=1)
  gather_idx = batch['residx_atom14_to_atom37'].unsqueeze(-1).repeat([1]*(len(atom37_data.size())-1) + [atom37_data.size(-1)])
  atom14_data = torch.gather(atom37_data, 1, gather_idx)
  if len(atom37_data.shape) == 2:
    atom14_data = atom14_data * batch['atom14_atom_exists'].type_as(atom14_data)
  elif len(atom37_data.shape) == 3:
    atom14_data = atom14_data * batch['atom14_atom_exists'][:, :,
                                               None].type_as(atom14_data)
  return atom14_data
...
def atom37_to_frames(
  ...
  aatype = aatype.view(-1) # jnp.reshape(aatype, [-1])
  all_atom_positions = all_atom_positions.view(-1, 37, 3) # jnp.reshape(all_atom_positions, [-1, 37, 3])
  all_atom_mask = all_atom_mask.view(-1, 37) #jnp.reshape(all_atom_mask, [-1, 37])
  ...
  # shape (N, 8, 3)
  restype_rigidgroup_base_atom37_idx = restype_rigidgroup_base_atom37_idx_.to(aatype.device)
  ...
  gather_idx = residx_rigidgroup_base_atom37_idx.view(aatype.shape[0], -1).unsqueeze(-1).repeat(1, 1, 3)
  base_atom_pos = torch.gather(all_atom_positions, 1, gather_idx)
  # [nres, frames, 3 atomes, xyz]
  base_atom_pos = base_atom_pos.view(aatype.shape[0], 8, 3, 3)
  ...
  # Compute the Rigids.
  # 这里不用 chain up 因为这里的 all_atom_positions 本身坐标的原子就已经被 chain up，所以只需要根据坐标把frame算出就行
  ...
  # (N, 8)
  restype_rigidgroup_mask = restype_rigidgroup_mask_.to(aatype.device)
  group_exists = restype_rigidgroup_mask[aatype] #utils.batched_gather(restype_rigidgroup_mask, aatype)
  ...
  #     residx_rigidgroup_base_atom37_idx,
  #     batch_dims=1)
  gather_idx = residx_rigidgroup_base_atom37_idx.view(aatype.shape[0], -1)
  gt_atoms_exist = torch.gather(all_atom_mask.float(), 1, gather_idx)
  gt_atoms_exist = gt_atoms_exist.view(aatype.shape[0], 8, 3)
  ...
  # rots = torch.eye(3).to(aatype.device).float() #[None].repeat(8, 1, 1) #np.tile(np.eye(3, dtype=np.float32), [8, 1, 1])
  # rots[0, 0] = -1
  # rots[2, 2] = -1
  ...
  # restype_rigidgroup_is_ambiguous = torch.zeros([21, 8]).float().to(aatype.device)
  # restype_rigidgroup_rots = torch.eye(3).to(aatype.device).float()[None, None].repeat(21, 8, 1, 1) #np.tile(np.eye(3, dtype=np.float32), [21, 8, 1, 1])
  ...
  restype_rigidgroup_is_ambiguous = restype_rigidgroup_is_ambiguous_.to(aatype.device)
  restype_rigidgroup_rots = restype_rigidgroup_rots_.to(aatype.device)
  ...
  gt_frames_flat12 = gt_frames_flat12.view(aatype_in_shape + (8, 12))
    # jnp.reshape(gt_frames_flat12, aatype_in_shape + (8, 12))
  gt_exists = gt_exists.view(aatype_in_shape + (8,))
    # jnp.reshape(gt_exists, aatype_in_shape + (8,))
  group_exists = group_exists.view(aatype_in_shape + (8,))
    # jnp.reshape(group_exists, aatype_in_shape + (8,))
  gt_frames_flat12 = gt_frames_flat12.view(aatype_in_shape + (8, 12))
    # jnp.reshape(gt_frames_flat12, aatype_in_shape + (8, 12))
  residx_rigidgroup_is_ambiguous = residx_rigidgroup_is_ambiguous.view(aatype_in_shape + (8,))
    # jnp.reshape(residx_rigidgroup_is_ambiguous,
    #                                            aatype_in_shape + (8,))
  alt_gt_frames_flat12 = alt_gt_frames_flat12.view(aatype_in_shape + (8, 12,))
    # jnp.reshape(alt_gt_frames_flat12,
...
def atom37_to_torsion_angles(
    aatype: torch.FloatTensor,  # (B, N)
    all_atom_pos: torch.FloatTensor,  # (B, N, 37, 3)
    all_atom_mask: torch.FloatTensor,  # (B, N, 37)
  ...
  aatype = aatype.clamp(max = 20)

  # Compute the backbone angles.
  num_batch, num_res = aatype.size()

  pad = torch.zeros((num_batch, 1, 37, 3)).float().to(aatype.device)
  # all previous atoms
  prev_all_atom_pos = torch.cat([pad, all_atom_pos[:, :-1, :, :]], dim=1)

  pad = torch.zeros((num_batch, 1, 37)).float().to(aatype.device)
  ...
  # ??? prev_all_atom_pos.shape (B, N-1, 4, 3), all_atom_pos.shape (B, N, 4, 3)
  ...
  B, N = aatype.size()
  indices = aatype.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 4, 4).view(-1, 4, 4)
  # [batch, num_res, chis=4, atoms=4]
  atom_indices = torch.gather(chi_atom_indices, 0, indices).view(B, N, 4, 4)

  # Gather atom positions. Shape: [batch, num_res, chis=4, atoms=4, xyz=3].
  indeces = atom_indices.view(B, N, -1).unsqueeze(-1).repeat(1, 1, 1, 3)
  chis_atom_pos = torch.gather(all_atom_pos, 2, indeces).view(B, N, 4, 4, 3)
  ...
  indeces = atom_indices.view(B, N, -1)
  chi_angle_atoms_mask = torch.gather(all_atom_mask, 2, indeces).view(B, N, 4, 4)
  ...
  chis_mask = chis_mask * (chi_angle_atoms_mask).float()
  ...
  # local pos for fourth atoms in frame
  ...
  # z/x ? 如果是 N(X), Ca(O), C(Y) 是这样的，但是这里几个原子之间的位置交换所以不再是原本的 z/x
  ...
  # Mirror psi, because we computed it from the Oxygen-atom. ??? L-amino acid
  torsion_angles_sin_cos = torsion_angles_sin_cos * torch.FloatTensor(
      [1., 1., -1., 1., 1., 1., 1.])[None, None, :, None].to(aatype.device)

  # Create alternative angles for ambiguous atom names.
  # get indices for posible residue
  chi_pi_periodic = torch.FloatTensor(residue_constants.chi_pi_periodic).to(aatype.device)
  indeces = aatype.unsqueeze(-1).repeat(1, 1, 4).view(-1, 4)
  chi_is_ambiguous = torch.gather(chi_pi_periodic, 0, indeces).view(B, N, 4)

  mirror_torsion_angles = torch.cat(
      [torch.ones((num_batch, num_res, 3)).to(aatype.device),
...
def torsion_angles_to_frames(
  ...
  m = torch.gather(residue_constants.restype_rigid_group_default_frame.to(aatype.device), 0,
                   aatype[..., None, None, None].repeat(1, 8, 4, 4))
  ...
  # 这里注意 这里的 torsion angle 是基于每个独立坐标系的，并不是在一个残基为整体的坐标系下的
  sin_angles = torsion_angles_sin_cos[..., 0]
  cos_angles = torsion_angles_sin_cos[..., 1]

  # insert zero rotation for backbone group.
  num_residues, = aatype.shape
  sin_angles = torch.cat([torch.zeros((num_residues, 1)).to(aatype.device), sin_angles],
                               dim=-1)
  cos_angles = torch.cat([torch.ones((num_residues, 1)).to(aatype.device), cos_angles],
                               dim=-1)
  ...
  # Apply rotations to the frames. Independent frame here have not rely on previous frame
  ...
  chi2_frame_to_frame = r3.tree_map_rigids(lambda x: x[:, 5], all_frames)
  chi3_frame_to_frame = r3.tree_map_rigids(lambda x: x[:, 6], all_frames)
  chi4_frame_to_frame = r3.tree_map_rigids(lambda x: x[:, 7], all_frames)
  chi1_frame_to_backb = r3.tree_map_rigids(lambda x: x[:, 4], all_frames)

  # frome chi frame to backbone, so the order is r3.rigids_mul_rigids(backb, chi)
  ...
  # identity, omega, phi, psi, chi2, chi3, chi4
  def _concat_frames(xall, x5, x6, x7):
    return torch.cat(
        [xall[:, 0:5], x5[:, None], x6[:, None], x7[:, None]], dim=-1)
  
  def concat_rigid(rg1, rg2, rg3, rg4):
    return r3.Rigids(
            rot = r3.Rots(
              _concat_frames(rg1.rot.xx, rg2.rot.xx, rg3.rot.xx, rg4.rot.xx),
              _concat_frames(rg1.rot.xy, rg2.rot.xy, rg3.rot.xy, rg4.rot.xy),
              _concat_frames(rg1.rot.xz, rg2.rot.xz, rg3.rot.xz, rg4.rot.xz),
              _concat_frames(rg1.rot.yx, rg2.rot.yx, rg3.rot.yx, rg4.rot.yx),
              _concat_frames(rg1.rot.yy, rg2.rot.yy, rg3.rot.yy, rg4.rot.yy),
              _concat_frames(rg1.rot.yz, rg2.rot.yz, rg3.rot.yz, rg4.rot.yz),
              _concat_frames(rg1.rot.zx, rg2.rot.zx, rg3.rot.zx, rg4.rot.zx),
              _concat_frames(rg1.rot.zy, rg2.rot.zy, rg3.rot.zy, rg4.rot.zy),
              _concat_frames(rg1.rot.zz, rg2.rot.zz, rg3.rot.zz, rg4.rot.zz),
            ),
            trans = r3.Vecs(
              _concat_frames(rg1.trans.x, rg2.trans.x, rg3.trans.x, rg4.trans.x),
              _concat_frames(rg1.trans.y, rg2.trans.y, rg3.trans.y, rg4.trans.y),
              _concat_frames(rg1.trans.z, rg2.trans.z, rg3.trans.z, rg4.trans.z),
            )
        )

  all_frames_to_backb = concat_rigid(
      all_frames,
      chi2_frame_to_backb,
      chi3_frame_to_backb,
      chi4_frame_to_backb)
  ...
  # chain up backb_to_global and all_frames_to_backb, 
  # first: all_frames_to_backb; second: backb_to_global
  # shape (N, 8)
  all_frames_to_global = r3.rigids_mul_rigids(
      r3.tree_map_rigids(lambda x: x[:, None], backb_to_global),
      all_frames_to_backb)
...
def frames_and_literature_positions_to_atom14_pos(
  ...
  # get groups for 14 atoms in each residue
  residx_to_group_idx = torch.gather(residue_constants.restype_atom14_to_rigid_group.to(aatype.device), 0, 
                                     aatype.unsqueeze(-1).repeat(1, 14) )
  # one-hot tranformation
  group_mask = F.one_hot(
      residx_to_group_idx, num_classes=8)  # shape (N, 14, 8)
  # all_frames_to_global shape (N, 1, 8)
  # group_mask shape (N, 14, 8)
  # torch.sum -> (N, 14) rigid 
  ## 注意这里的 N x 14 个rigid 是存储在 一个 rigid 里的，就是每个 r.rot.xx 的维度是 N x 14
  # 从 torsion_angles_to_frames 知道, 这里的 all_frames_to_global 已经被串起来了, 就是每一个 rigid 转化都是基于前一个的 
  # after sum all frame are transformed to a global transformation rigid, 
  # all you need is use Nx14 rigids transform the literature atoms
  map_atoms_to_global = r3.tree_map_rigids(
      lambda x: torch.sum(x[:, None, :] * group_mask, dim=-1),
      all_frames_to_global)
  # map_atoms_to_global = to_global_fn(all_frames_to_global)

  # Gather the literature atom positions for each residue.
  # r3.Vecs with shape (N, 14)
  lit_positions = r3.vecs_from_tensor(
    torch.gather(residue_constants.restype_atom14_rigid_group_positions.to(aatype.device), 0, 
                                     aatype[..., None, None].repeat(1, 14, 3) ))
  ...
  # Gather all mask for each residue.
  mask = torch.gather(residue_constants.restype_atom14_mask.to(aatype.device), 0, 
                                     aatype.unsqueeze(-1).repeat(1, 14) )
  pred_positions = r3.tree_map_vecs(lambda x: x * mask, pred_positions)
...
def extreme_ca_ca_distance_violations(
  ...
  ca_ca_distance = torch.sqrt(
      1e-6 + torch.sum(squared_difference(this_ca_pos, next_ca_pos), dim=-1))
...
def between_residue_bond_loss_batch(
  ...
  has_no_gap_mask = ((residue_index[:, 1:] - residue_index[:, :-1]) == 1.0).float() #.astype(jnp.float32)
  # import pdb; pdb.set_trace()
  # Compute loss for the C--N bond.
  c_n_bond_length = torch.sqrt(
      1e-6 + torch.sum(squared_difference(this_c_pos, next_n_pos), dim=-1))

  # The C-N bond to proline has slightly different length because of the ring.
  next_is_proline = (
      aatype[:, 1:] == residue_constants.resname_to_idx['PRO']).float() #.astype(jnp.float32)
  gt_length = (
  ...
  # c_n_loss = torch.sum(mask * c_n_loss_per_residue, 1) / (torch.sum(mask, 1) + 1e-6)
  c_n_loss = sum(torch.sum(mask * c_n_loss_per_residue, 1) / (torch.sum(mask, 1) + 1e-6)) / mask.shape[0]
  c_n_violation_mask = mask * (
      c_n_bond_length_error > (tolerance_factor_hard * gt_stddev))

  # Compute loss for the angles.
  ca_c_bond_length = torch.sqrt(1e-6 + torch.sum(
      squared_difference(this_ca_pos, this_c_pos), dim=-1))
  n_ca_bond_length = torch.sqrt(1e-6 + torch.sum(
      squared_difference(next_n_pos, next_ca_pos), dim=-1))
  ...
  ca_c_n_loss = sum(torch.sum(mask * ca_c_n_loss_per_residue, 1) / (torch.sum(mask, 1) + 1e-6)) / mask.shape[0]
  ...
  c_n_ca_loss_per_residue = F.relu(c_n_ca_cos_angle_error - tolerance_factor_soft * gt_stddev)
  mask = this_c_mask * next_n_mask * next_ca_mask * has_no_gap_mask
  # c_n_ca_loss = torch.sum(mask * c_n_ca_loss_per_residue) / (torch.sum(mask) + 1e-6)
  c_n_ca_loss = sum(torch.sum(mask * c_n_ca_loss_per_residue, 1) / (torch.sum(mask, 1) + 1e-6)) / mask.shape[0]
  ...
  violation_mask = torch.max(
      torch.stack([c_n_violation_mask,ca_c_n_violation_mask,c_n_ca_violation_mask], 1), dim=1)[0]
  violation_mask = torch.max(torch.stack(
      [F.pad(violation_mask, (0, 1)),
      F.pad(violation_mask, (1, 0))], 1), dim=1)[0]
...
def between_residue_bond_loss(
  ...
  violation_mask = torch.max(
      torch.stack([c_n_violation_mask,
                 ...
                 c_n_ca_violation_mask]), dim=0)[0]
  violation_mask = torch.max(torch.stack(
      [F.pad(violation_mask, (0, 1)),
      F.pad(violation_mask, (1, 0))]), dim=0)[0]
...
def between_residue_clash_loss_batch(
    atom14_pred_positions, #: jnp.ndarray,  # (B, N, 14, 3)
    atom14_atom_exists, #: jnp.ndarray,  # (B, N, 14)
    atom14_atom_radius, #: jnp.ndarray,  # (B, N, 14)
    residue_index, #: jnp.ndarray,  # (B, N)
  ...
  c_one_hot = F.one_hot(torch.LongTensor([2]), num_classes=natoms).to(atom14_pred_positions.device).squeeze(0)
  n_one_hot = F.one_hot(torch.LongTensor([0]), num_classes=natoms).to(atom14_pred_positions.device).squeeze(0)
  ...
  # cys_sg_one_hot = F.one_hot(torch.LongTensor([cys_sg_idx]), num_classes=natoms).to(atom14_pred_positions.device).squeeze(0)
  ...
  # shape ()
  mean_loss = (torch.sum(dists_to_low_error)
               / (1e-6 + torch.sum(dists_mask)))

  # Compute the per atom loss sum.
  # shape (B, N, 14)
  per_atom_loss_sum = (torch.sum(dists_to_low_error, dim=[1, 3]) +
                       torch.sum(dists_to_low_error, dim=[2, 4]))
  ...
  # shape (B, N, 14)
  per_atom_clash_mask = torch.stack([clash_mask.max(3)[0].max(1)[0], clash_mask.max(4)[0].max(1)[0]], 1).max(1)[0]
...
def between_residue_clash_loss(
  ...
  # Compute the per atom loss sum.
  # shape (N, 14)
  per_atom_loss_sum = (torch.sum(dists_to_low_error, dim=[0, 2]) +
                       torch.sum(dists_to_low_error, dim=[1, 3]))
  # print(per_atom_loss_sum)
  # import pdb; pdb.set_trace()
  ...
  # Compute the per atom clash.
  # shape (N, 14)
  per_atom_clash_mask = torch.stack([clash_mask.max(2)[0].max(0)[0], clash_mask.max(3)[0].max(0)[0]]).max(0)[0]
...
def within_residue_violations_batch(
    atom14_pred_positions, #: jnp.ndarray,  # (B, N, 14, 3)
    atom14_atom_exists, #: jnp.ndarray,  # (B, N, 14)
    atom14_dists_lower_bound, #: jnp.ndarray,  # (B, N, 14, 14)
    atom14_dists_upper_bound, #: jnp.ndarray,  # (B, N, 14, 14)
  ...
  # shape (B, N, 14, 14)
  dists_masks = (1. - torch.eye(natoms, natoms)[None, None]).to(atom14_atom_exists.device)
  dists_masks = dists_masks * (atom14_atom_exists[:, :, :, None] *
  ...
  # Compute the per atom loss sum.
  # shape (B, N, 14)
  per_atom_loss_sum = (torch.sum(loss, dim=2) +
                       torch.sum(loss, dim=3))
  ...
  # shape (B, N, 14)
  per_atom_violations = torch.stack([violations.max(2)[0], violations.max(3)[0]], 1).max(1)[0]
...
def within_residue_violations(
  ...
  # Compute the mask for each residue.
  # shape (N, 14, 14)
  dists_masks = (1. - torch.eye(natoms, natoms)[None]).to(atom14_atom_exists.device)
  dists_masks = dists_masks * (atom14_atom_exists[:, :, None] *
                  atom14_atom_exists[:, None, :])
  ...
  # Compute the per atom loss sum.
  # shape (N, 14)
  per_atom_loss_sum = (torch.sum(loss, dim=1) +
  ...
  # Compute the per atom violations.
  # shape (N, 14)
  per_atom_violations = torch.stack([violations.max(1)[0], violations.max(2)[0]]).max(0)[0]
...
def find_optimal_renaming(
  ...
  # shape (N)
  alt_naming_is_better = (alt_per_res_lddt < per_res_lddt).float() #.astype(jnp.float32)
...
def frame_aligned_point_error(
  ...
  # transform pred position to pred_frame, namely view pred_frames the local frame
  # r3.Vecs (num_frames, num_positions)
  local_pred_pos = r3.rigids_mul_vecs(
      r3.tree_map_rigids(lambda r: r[:, None], r3.invert_rigids(pred_frames)),
      r3.tree_map_vecs(lambda x: x[None, :], pred_positions))

  # Compute array of target positions in the target frames.
  # transform target position to target_frame, namely view target_frames the local frame
  # r3.Vecs (num_frames, num_positions)
  local_target_pos = r3.rigids_mul_vecs(
      r3.tree_map_rigids(lambda r: r[:, None], r3.invert_rigids(target_frames)),
      r3.tree_map_vecs(lambda x: x[None, :], target_positions))
  ...
  if l1_clamp_distance:
    error_dist = error_dist.clamp(0, l1_clamp_distance) # jnp.clip(error_dist, 0, l1_clamp_distance)

  normed_error = error_dist / length_scale
  normed_error = normed_error * frames_mask.unsqueeze(-1) #jnp.expand_dims(frames_mask, axis=-1)
  normed_error = normed_error * positions_mask.unsqueeze(-2) #jnp.expand_dims(positions_mask, axis=-2)

  normalization_factor = (
      torch.sum(frames_mask.float(), dim=-1) *
      torch.sum(positions_mask.float(), axis=-1))
  return (torch.sum(normed_error, dim=(-2, -1)) /
...
def _make_renaming_matrices():
  ...
  all_matrices = {res: torch.eye(14).float() for res in restype_3}
  for resname, swap in residue_constants.residue_atom_renaming_swaps.items():
    ...
    for source_atom_swap, target_atom_swap in swap.items():
      ...
      renaming_matrix = torch.zeros((14, 14)).float()
      for index, correspondence in enumerate(correspondences):
        renaming_matrix[index, correspondence] = 1.
    all_matrices[resname] = renaming_matrix.float()
...
def get_alt_atom14(aatype, positions, mask):
  ...
  # shape (num_res, 14, 14)
  renaming_transform = torch.gather(torch.FloatTensor(RENAMING_MATRICES, 0, aatype))

  positions = r3.tree_map_vecs(lambda x: x[:, :, None], positions)
  alternative_positions = r3.tree_map_vecs(
      lambda x: torch.sum(x, dim=1), positions * renaming_transform)
...
def rebiuld_from_atom_crd(crd_list, chain="A", filename='test.pdb', natom=4, natom_dict=None):
