...
def tree_map_rigids(f, r):
  return Rigids(
    Rots(
      f(r.rot.xx), f(r.rot.xy), f(r.rot.xz), 
      f(r.rot.yx), f(r.rot.yy), f(r.rot.yz), 
      f(r.rot.zx), f(r.rot.zy), f(r.rot.zz)),
    Vecs(f(r.trans.x), f(r.trans.y), f(r.trans.z)),
  )

def tree_map_vecs(f, v):
  return Vecs(f(v.x), f(v.y), f(v.z))


def stack_rigids(rs):
  return Rigids(
    Rots(
      torch.stack([r.rot.xx for r in rs]),
      torch.stack([r.rot.xy for r in rs]),
      torch.stack([r.rot.xz for r in rs]),
      torch.stack([r.rot.yx for r in rs]),
      torch.stack([r.rot.yy for r in rs]),
      torch.stack([r.rot.yz for r in rs]),
      torch.stack([r.rot.zx for r in rs]),
      torch.stack([r.rot.zy for r in rs]),
      torch.stack([r.rot.zz for r in rs])),
    Vecs(
      torch.stack([r.trans.x for r in rs]),
      torch.stack([r.trans.y for r in rs]),
      torch.stack([r.trans.z for r in rs]))
  )

def stack_vecs(vs):
  return Vecs(
      torch.stack([v.x for v in vs]),
      torch.stack([v.y for v in vs]),
      torch.stack([v.z for v in vs]))
...
def rigids_to_quataffine_m(r: Rigids) -> quat_affine.QuatAffine:
  """Convert Rigids r into QuatAffine, inverse of 'rigids_from_quataffine'."""
  rot = [[r.rot.xx, r.rot.xy, r.rot.xz],
          [r.rot.yx, r.rot.yy, r.rot.yz],
          [r.rot.zx, r.rot.zy, r.rot.zz]]
...
def rigids_to_tensor_flat12(
  ...
  """Flat12 encoding: rotation matrix (9 floats) + translation (3 floats)."""
  return torch.stack(list(r.rot) + list(r.trans), axis=-1)
...
def vecs_dot_vecs(v1: Vecs, v2: Vecs):
...
def vecs_from_tensor(x  # shape (..., 3)
                    ):  # shape (...)
...
def vecs_squared_distance(v1, v2):
...
def vecs_to_tensor(v  # shape (...)
                  ):  # shape(..., 3)
  """Converts 'v' to tensor with shape 3, inverse of 'vecs_from_tensor'."""
  return torch.stack([v.x, v.y, v.z], axis=-1)


if __name__ == "__main__":
  vecs_ch1 = torch.rand(1, 3)
  vecs1 = vecs_from_tensor(vecs1)

  vecs_ch2 = torch.rand(1, 3)
  vecs2 = vecs_from_tensor(vecs_ch2)
  print(rots_from_two_vecs(vecs1, vecs2))
